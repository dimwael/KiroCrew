...
__all__ = ["safe_terminal_line", "safe_terminal_text"]
...
def safe_terminal_text(value: str) -> str:
    ...
    return _bounded(_TERMINAL_CTRL_RE.sub("", value))


def safe_terminal_line(value: str) -> str:
    """Return :func:`safe_terminal_text` output confined to ONE terminal line.

    For renderers that print a prefix per line (``✅``/``⚠️``/``❌``), a preserved
    newline in untrusted text would start an unprefixed line that reads as the
    CLI's own output. Newlines are rendered as the visible ``\\x0a`` literal (the
    convention ``doctor_deadpath`` already uses) before the length cap, so the cap
    still applies to what is actually printed. Carriage returns are already in the
    C0 range the shared pattern strips.
    """
    return _bounded(_TERMINAL_CTRL_RE.sub("", value).replace("\n", "\\x0a"))


def _bounded(cleaned: str) -> str:
