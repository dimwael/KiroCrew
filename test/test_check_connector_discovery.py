...
class TestSingleContractBoundary:
    ...
    def test_source_status_and_evidence_tier_value_sets_are_distinct(self):
        # Guards the documented distinction: the two share only "unverified".
        assert set(gate.SOURCE_STATUS_VALUES) != set(gate.EVIDENCE_TIER_VALUES)
        shared = set(gate.SOURCE_STATUS_VALUES) & set(gate.EVIDENCE_TIER_VALUES)
        assert shared == {"unverified"}
