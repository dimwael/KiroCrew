...
- ``source_status``     -- manifest evidence axis; values user_required /
                           official_baseline / unverified
- ``source_kind``       -- manifest entry ``source.source_kind``; six-value enum
- ``schema_version``    -- manifest ``input_schema`` / ``output_schema``

``source_status`` is NOT ``evidence_tier``: the latter lives in
``catalog-evidence.json`` (values source_verified_strict /
search_snippet_or_partial / unverified) per campaign contract 6.4, is owned by
the conformance/evidence work stream, and is neither a manifest field nor a
discovery field. See connector-discovery.md for why the two must not be merged.
...
# The evidence-strength values of the DISTINCT field ``evidence_tier`` and the
# evidence-axis values of ``source_status`` are recorded here as reference only,
# to keep the single-contract distinction visible at the point a future reader
# might be tempted to add either check to this discovery gate. Neither is
# enforced here; both belong to other work streams.
SOURCE_STATUS_VALUES = ("user_required", "official_baseline", "unverified")
EVIDENCE_TIER_VALUES = (
    "source_verified_strict",
    "search_snippet_or_partial",
    "unverified",
)
