...
slug with another LIVE member is refused (409 `slug_collision`, in plain words -- "Nothing was fired. Another member, 'X', has the same short name ('slug') as 'Y', so they share files. Rename or fire that member first: it is on the Crew page if it is a crewmate, otherwise under Capabilities > Crews." -- the sentence says where the colliding member lives, since a plain crew is not on the roster the reader is looking at -- never the internal "member space" / "shorten to" vocabulary): `members/<slug>/`,
...
`members/<slug>/` where they are (a same-name hire later inherits none of it: the hire
copies afresh, the binding is checked by member, the space is a new slug generation
only if the slug is free -- otherwise the hire is refused with `slug_collision`).
Whether that delete should refuse an enrolled crewmate and point at Fire, or clean
the way Fire cleans, is a product decision the maintainer has not taken; fire is the
member's verb and the only one that touches lived state.
...
`test_a_pre_existing_package_row_for_an_apps_file_is_restamped_as_the_apps`). The
