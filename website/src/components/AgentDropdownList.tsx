      ...
      {grouped.map(group => (
        <div key={group.id} role="presentation" className="flex flex-col" data-testid={`agent-group-${group.id}`}>
          {grouped.length > 1 && (
            ...
            // Sticky: the active row is scrolled into view on open, which can
            // carry a group's heading off the top while its rows stay visible --
            // the two groups then read as one list under the other's name.
